...
--
-- 이미 탈퇴한 회원의 번호는 건드리지 않는다. 2026-09-12 현재 운영 DB 는 런칭 전
-- 테스트 데이터뿐이라 소급 대상이 없다. 실데이터가 들어간 뒤에 이 함수가 바뀐다면
-- 그때는 소급 여부를 따로 판단해야 한다.
--
-- 딸린 결과: 번호가 지워지므로 "같은 번호로 재가입한 손님의 과거 이력 확인"은
-- 더 이상 되지 않는다. 런칭 전이라 실제 영향은 없고 점주 확인도 받았다.
-- 개인정보 처리방침 제4조 ③ 과 탈퇴 확인 문구도 이 동작에 맞춰 고쳐 뒀다.
