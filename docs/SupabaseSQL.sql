...
-- Includes manager shared tables/RLS plus current customer-app RPCs.
-- Not included: app_configs admin password storage, deprecated AI proxy quota tables, test data.
...
CREATE OR REPLACE FUNCTION public.use_my_coupon(p_session_token text, p_coupon_id integer)
RETURNS public.coupon_history LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_customer_id uuid; v_coupon public.coupon_history%ROWTYPE;
BEGIN
  v_customer_id := public.resolve_customer_session(p_session_token);
  IF v_customer_id IS NULL THEN RAISE EXCEPTION 'Invalid or expired customer session' USING ERRCODE = '28000'; END IF;
  UPDATE public.coupon_history SET is_used = true, used_at = now()
  WHERE id = p_coupon_id AND customer_id = v_customer_id AND is_used = false AND (valid_until IS NULL OR valid_until >= now())
  RETURNING * INTO v_coupon;
  IF v_coupon.id IS NULL THEN RAISE EXCEPTION 'Coupon not found, already used, or expired' USING ERRCODE = '02000'; END IF;
  RETURN v_coupon;
END;
$$;
