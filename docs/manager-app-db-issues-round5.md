...
### 이미 탈퇴한 회원은 건드리지 않았습니다

되돌릴 수 없는 일괄 변경이고 합의된 바가 없어서 함수만 바꿨습니다.
**소급 마스킹을 할지 판단 부탁드립니다.** 보관 최소화 관점에서는 하는 쪽이 맞지만,
매장 적립 내역 대조에 과거 번호가 필요하다면 남기는 선택도 있습니다.
하기로 하면 `UPDATE ... WHERE deleted_at IS NOT NULL` 한 줄짜리 마이그레이션으로
올리겠습니다.

### 딸린 결과 — 재가입 시 이력이 끊깁니다

번호가 지워지므로 **"같은 번호로 재가입한 손님의 과거 스탬프·쿠폰 확인"이 더 이상
되지 않습니다.** 처리방침 제4조 ③ 이 그 보관 목적을 근거로 적혀 있어서 같이 고쳤고,
탈퇴 확인 다이얼로그에도 문구를 넣었습니다.

> 탈퇴하면 닉네임과 생일이 삭제되고 휴대폰번호는 알아볼 수 없게 바뀝니다.
> 같은 번호로 다시 가입할 수는 있지만, 지금까지 모은 스탬프와 쿠폰은 새 계정으로
> 이어지지 않습니다.

스탬프를 9개쯤 모아 둔 회원에게는 "되돌릴 수 없습니다"보다 이게 중요한 정보입니다.
**매장 응대에 영향이 있는 변화라 점주 확인이 필요합니다** — 지금까지는 재가입 손님의
이력을 번호로 찾아 줄 수 있었는데, 적용 후에는 불가능합니다.
...
| 15 | 기존 탈퇴 회원 소급 마스킹 여부 | **매니저·점주** | ❓ 신규 판단 요청 (§4) |
| 16 | 재가입 시 이력 단절 — 매장 응대 영향 확인 | **점주** | ❓ 신규 확인 요청 (§4) |
